...
var controllerUser = {
    ...
    loginUser: async (req,res) => {
        connection.query('SELECT * FROM users WHERE username = BINARY ?',[req.body.username], async (err, result) => {
            if(err) return res.status(500).send({ message: 'error al cargar' })
            if(!result) return res.status(404).send({ message: 'no se a podido guardar el cliente' })
            if(JSON.stringify(result) == '[]') return res.status(400).send( {error: true, message: 'Credenciales invalidas'})
            else {
                const validPassword = await bcrypt.compare(req.body.password, result[0].password);
                if(!validPassword) return res.status(400).send({ error: true, message: 'Credenciales invalidas' })
